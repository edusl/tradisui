...
* http://jqueryui.com
* Includes: focusable.js, keycode.js, widgets/datepicker.js
...
$.extend( Datepicker.prototype, {
	...
	_generateHTML: function( inst ) {
		...
		prev = ( this._canAdjustMonth( inst, -1, drawYear, drawMonth ) ?
		  ...
		  " title='" + prevText + "'><svg class='icon'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#arrow-simple'></use></svg></a>" :
		  ( hideIfNoPrevNext ? "" : "<a class='ui-datepicker-prev ui-corner-all ui-state-disabled' title='" + prevText + "'><svg class='icon'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#arrow-simple'></use></svg></a>" ) );
		...
		next = ( this._canAdjustMonth( inst, +1, drawYear, drawMonth ) ?
		  ...
		  " title='" + nextText + "'><svg class='icon rotate-180'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#arrow-simple'></use></svg></a>" :
		  ( hideIfNoPrevNext ? "" : "<a class='ui-datepicker-next ui-corner-all ui-state-disabled' title='" + nextText + "'><svg class='icon rotate-180'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='#arrow-simple'></use></svg></a>" ) );
                    ...
                    a.each(e, function(b, d) {
                        if (d = a.trim(d), 0 === d.length || a.formUtils.hasLoadedModule(d)) g();
                            ...
                            "function" == typeof define && define.amd ? require([e + (".dev.js" === e.slice(-7) ? h : "")], g) : (f.type = "text/javascript", f.onload = g, f.src = e + (".dev.js" === e.slice(-7) ? h : ""), f.onerror = function() {
                                a.formUtils.warn("Unable to load form validation module " + e), g()
...
$(document).ready(function() {
  ...
  */
    ...
    function openLateral(modalContent) {
      ...
      $('.overlay').fadeIn('slow');
      $(".modal-embed").empty();
      $('.'+ modalContent).clone().appendTo($(".modal-lateral-content"));
  ...
  */
	  ...
	  function openModal(modalContent) {
	    ...
	    //$('.modal-top').html('<a class=\'modal-close\'><svg><use xmlns:xlink=\'http://www.w3.org/1999/xlink\' xlink:href=\'#close\'></use></svg></a>');
	    $('.'+modalContent+' .modal-content').append('<a class=\'modal-close\'><svg><use xmlns:xlink=\'http://www.w3.org/1999/xlink\' xlink:href=\'#close\'></use></svg></a>'); 
    ...
    */
    ...
    /*
      GO BACK
      Add "link--back" class to return before´s page
    */
    ...
    */
    ...
    var mn = $(".menu-anchor");
    mns = "menu-anchor--fixed";
    hdr = $('.main-header').height();
    ...
    */
    ...
    $.each($(".rb-group"), function() {
        $.each($("input[type=radio]:checked"), function() {
            conta++;
        });
        if (conta < 1) {
            //$(this).('input').filter(':first').prop( "checked", true );
        }
    ...
    */
    ...
    /*
      CALENDAR
      Add "input--calendar" to show datepicker
    */

    $(".input--expedition").datepicker({
        changeMonth: true,
        changeYear: true,
    ...
    */
    ...
    // Adding zeros if number is below five numbers
    function prependZeros(num) {
        var str = ("" + num);
        return (Array(Math.max(6 - str.length, 0)).join("0") + str);
    }
    ...
    $('select').on('change', function() {
    	...
    	if ($(this).attr('data-load')) {
            ...
            console.log('loading ' + loadJson + ' on ' + $(this).attr('data-depend') + ' valor=' + $(this).val());
    ...
    function cargaMuniRegi(value, depend, action) {
        ...
        if ($('#' + depend).attr('data-cache')) {
            ...
            $.getJSON("/api/location/registralTowns?province=" + value, function(jsonData) {
                ...
                if ($item > 1) {
                    $('#' + depend).html('<option value="">Seleccione un municipio</option>' + cb);
                } else {
                    $('#' + depend).html(cb);
    ...
    function cargaMuni(value, depend, action) {
        ...
        console.log('cache=' + $('#' + depend).data('cache'));
    ...
    function cargaRegi(value, depend) {
        ...
        $.getJSON("/api/registries?province=" + ($('.prov:enabled').val()) + "&town=" + ($('.muni:enabled').val()), function(jsonData) {

            //cb = '<option value="">Seleccione un registro</option>';
            cb = "";
    ...
    function cargaRegiProv(value, depend) {
        $('#' + depend).children('option').remove();
        $('#' + depend).addClass('select--loading');
        $('#' + depend).html('<option>Cargando registros ...</option>');


        $.getJSON("/api/provinceRegistries?province=" + value, function(jsonData) {
            ...
            if ($item > 1) {
                $('#' + depend).html('<option value="">Seleccione una secci&oacute;n</option>' + cb);
            } else {
                $('#' + depend).html(cb);
            }
    ...
    function cargaSecc(value, depend) {
        ...
        $.getJSON("/api/registrySections?registryCode=" + ($('.regi:enabled').val()), function(jsonData) {
    ...
    */
    ...
    $('#checkDir').click(function() {
        ...
        console.log('formated dir: '+ $formattedDir);
    ...
    function checkDir(value) {
      ...
      $.getJSON(("/api/geocoding/addresses?q=" + value), function(jsonData) {
    	  
    	$('.od-result').html('');
    	 
    	if(jsonData < 1) {
    		$('.od-result').html('Sin resultados');
    	}
 		
  		$.each(jsonData, function(i, data) {
            $('.od-result').append('<div class="rb-option"><label class="rb-label" for="'+data.code+'"><input class="invisible radiobutton" id="'+data.code+'" type="radio" name="od-finca"><span class="rb-pick"></span><span class="rb-text">'+data.name+'</span><span class="rb-line"></span></label>');
        });	
  		
        $('.od-action').append('<a class="btn btn--primary btn--small" id="od-sFinca">Aceptar</a>');
    ...
    $(document).on('click', '#od-sFinca', function() {
    	var $odselect = $('input[name="od-finca"]:checked').attr('id');
    ...
    // calling to svg-sprite #6 
    $.get("#staticServerUri#/img/common/svg/svg-symbols.svg", function(data) {
